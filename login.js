...
// 1) Check that the name and password match a contact
  if (user == undefined){
    //res.send(`<h1> Invaild Username Or Incorrect Passward </h1>`);
    res.send(`<form action="/login" method="get">
    <input type="submit" value="Invaild Username or Incorrect Passward, Please Click Here to Try Again">
    </form>`);
  }
  ...
  // we dynamically create a form with a hidden field that contains the token - secret-token (no need to change this)
      ...
      res.send(form);
        ...
        <input type="submit" value="Sorry It Is NOT an Admin Account, Please Click Here to Try Again.">
        </form>`;  
        res.send(form);
      }
}});
